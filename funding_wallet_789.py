...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cc_wallet_789.log'),
...
class FundingRateProcessor:
    ...
    def load_environment(self) -> None:
        ...
        try:
            ...
            self.api_key = os.getenv("API_KEY5")
            self.api_secret = os.getenv("API_SECRET5")
            
            if not self.api_key or not self.api_secret:
                raise ConfigError(
                    f"Missing Binance API credentials. "
                    f"API_KEY5: {'Set' if self.api_key else 'NOT SET'}, "
                    f"API_SECRET5: {'Set' if self.api_secret else 'NOT SET'}"
                )
