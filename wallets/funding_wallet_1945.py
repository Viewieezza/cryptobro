...
from core.binance_get import get_asset_price, get_coinm_position_risk, get_usdtm_position_risk
import time
import base64
import json
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(__file__), '..', 'logs', 'cc_wallet_1945.log')),
...
class FundingRateProcessor:
    def __init__(self):
        ...
        self.worksheet_title = 'Funding Rate'
    ...
    def load_environment(self) -> None:
        ...
        try:
            ...
            self.api_secret = os.getenv("API_SECRET2")
            
            if not self.api_key or not self.api_secret:
                raise ConfigError(
                    f"Missing Binance API credentials. "
                    f"API_KEY2: {'Set' if self.api_key else 'NOT SET'}, "
                    f"API_SECRET2: {'Set' if self.api_secret else 'NOT SET'}"
                )
    ...
    def should_process_row(self, wallet: str, start_date: str, end_date: str) -> bool:
        """Check if a row should be processed based on business rules"""
        if wallet != self.wallet_name:
            return False
        
        if not start_date or start_date.strip() == "":
            logger.info(f"Skipping row: Start date (Column E) is empty")
            return False
        
        if end_date and end_date.strip() != "":
            logger.info(f"Skipping row: End date (Column F) already filled")
            return False
    ...
    def process_sheet_data(self, coinm_positions: List[Dict], usdtm_positions: List[Dict]) -> None:
        ...
        try:
            ...
            for row_index, row in enumerate(all_values[1:], start=2):
                try:
                    # Extract row data - adjusted column indices
                    wallet = row[3] if len(row) > 3 else ""           # Column D (index 3)
                    start_date = row[4] if len(row) > 4 else ""       # Column E (index 4)
                    end_date = row[5] if len(row) > 5 else ""         # Column F (index 5)
                    contract_symbol = row[6] if len(row) > 6 else ""  # Column G (index 6)
                    
                    # Check if row should be processed
                    if not self.should_process_row(wallet, start_date, end_date):
                        skipped_count += 1
                        continue
    ...
    def run(self) -> None:
        ...
        try:
            logger.info("Starting Funding Rate processor...")
...
def main():
    ...
    try:
        ...
        processor.run()
        logger.info("Funding Rate processor completed successfully")
    except Exception as e:
        logger.error(f"Funding Rate processor failed: {e}")
        exit(1)
