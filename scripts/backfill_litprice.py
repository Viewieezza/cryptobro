...
print("Initializing ccxt kucoin for historical data backfill...")
...
for i, row in enumerate(all_values):
    ...
    # Check if a backfill is needed (empty or invalid lit_price in Col H)
    old_price_str = row[7].strip() if len(row) > 7 else ""
    ...
    if not needs_backfill:
        continue # Skip rows that already have a valid price
    ...
    if len(row) > 0 and row[0].strip():
        ...
        try:
            ...
            if new_price > 0:
                print(f"Row {i+1} [{date_str}]: Backfilling {old_price_str or 'Empty'} -> New Price = {new_price}")
                ...
                print(f"Row {i+1}: Price is still 0.0, could not fetch.")
                
        except Exception as e:
            print(f"Row {i+1}: Error processing date {date_str} (skipped): {e}")

if cells_to_update:
    print(f"\nProceeding to batch update {len(cells_to_update)} backfilled rows in the Google Sheet...")
    sheet.batch_update(cells_to_update, value_input_option="USER_ENTERED")
    print("Successfully batch updated!")
else:
    print("\nNo rows need backfilling.")
