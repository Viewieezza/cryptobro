#!/usr/bin/env python3
"""
Backfill Sky Money (stUSDT) — ดึง balance ย้อนหลังทุกวัน → Google Sheet "test_SkyMoney"

ใช้ stUSDT contract: balanceOf → previewRedeem ณ historical block
ตั้งแต่วันฝากแรก (Jan-28-2026 08:57:11 UTC) จนถึงวันนี้

Usage:
    python3 scripts/backfill_sky_money.py
"""
import sys
import os
import json
...
# วันแรกที่ฝาก: Jan-28-2026 08:57:11 UTC
...
def _get_stusdt_balance_at_block(w3, block_number):
    """ดึง stUSDT balance (balanceOf → previewRedeem) ณ historical block"""
    from web3 import Web3

    wallet = Web3.to_checksum_address(WALLET)
    contract_addr = Web3.to_checksum_address(STUSDT_CONTRACT)
    contract = w3.eth.contract(address=contract_addr, abi=STUSDT_ABI)
    ...
    try:
        ...
        assets_raw = contract.functions.previewRedeem(shares).call(block_identifier=block_number)
        return assets_raw / (10 ** DECIMALS)
    except Exception as e:
        logging.warning(f"  stUSDT query failed at block {block_number}: {e}")
        return None
...
def main():
    print("=" * 60)
    print("Sky Money (stUSDT) — Historical Backfill")
    print("=" * 60)
    print(f"Contract : {STUSDT_CONTRACT}")
    ...
    for i, (date_str, target_ts) in enumerate(to_process):
        ...
        # Query stUSDT balance at that block
        balance = _get_stusdt_balance_at_block(w3, block_num)
        if balance is None:
            logging.warning(f"  ⚠️ Skip {date_str} — could not get balance")
            time_mod.sleep(0.5)
            continue
