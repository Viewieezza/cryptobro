#!/usr/bin/env python3
"""
Backfill USD.AI (USDai) — ดึงยอดคงเหลือย้อนหลังรายวันสำหรับ Wallet 2 → Google Sheet "USD.ai"

ดึงยอดคงเหลือ (USDai ใน wallet + sUSDai ใน vault) ณ บล็อกย้อนหลังในแต่ละวัน
ตั้งแต่วันแรกที่เริ่มฝาก (2026-05-20 13:30:15 GMT+7) จนถึงปัจจุบัน

Usage:
    python scripts/backfill_usdai.py
"""
import sys
import os
import json
...
# วันที่เริ่มฝากครั้งแรก: 2026-05-20 13:30:15 GMT+7 (= 06:30:15 UTC)
...
def _get_sheet():
    ...
    logging.info(f"Loaded GOOGLE_SHEET_ID: '{google_sheet_id}'")
...
def _ensure_headers(sheet):
    ...
    if len(row1) < 12 or not any(str(c).strip() for c in row1[:12]):
...
def _find_block_by_timestamp(w3, target_ts):
    """Binary search หา block ที่ timestamp ใกล้เคียง target_ts ที่สุด บน Arbitrum"""
    lo = 1
...
def _get_usdai_balances_at_block(w3, block_number):
    """ดึงยอดคงเหลือของ USDai และ sUSDai ณ บล็อกที่กำหนด"""
    from web3 import Web3
    ...
    try:
        # 1. ยอดใน wallet
        wallet_raw = usdai_contract.functions.balanceOf(wallet).call(block_identifier=block_number)
        wallet_bal = wallet_raw / (10 ** DECIMALS)

        # 2. ยอดใน staked vault
        shares_raw = susdai_contract.functions.balanceOf(wallet).call(block_identifier=block_number)
        vault_bal = 0.0
        if shares_raw > 0:
            try:
                assets_raw = susdai_contract.functions.convertToAssets(shares_raw).call(block_identifier=block_number)
                vault_bal = assets_raw / (10 ** DECIMALS)
            except Exception as e_vault:
                logging.warning(f"  convertToAssets failed at block {block_number}: {e_vault}, trying alternative previewRedeem")
                try:
                    abi_vault_alt = [
                    ...
                    assets_raw = c_alt.functions.previewRedeem(shares_raw).call(block_identifier=block_number)
                    vault_bal = assets_raw / (10 ** DECIMALS)
                except Exception as e_alt:
                    logging.error(f"  previewRedeem failed at block {block_number}: {e_alt}")
                    vault_bal = shares_raw / (10 ** DECIMALS)

        return wallet_bal + vault_bal

    except Exception as e:
        logging.warning(f"  USD.AI query failed at block {block_number}: {e}")
        return None
...
def main():
    print("=" * 60)
    print("USD.AI — Historical Backfill for Wallet 2")
    print("=" * 60)
    ...
    for i, (date_str, target_ts) in enumerate(to_process):
        ...
        balance = _get_usdai_balances_at_block(w3, block_num)
        if balance is None:
    ...
    try:
        ...
        # เรียงลำดับช่วง A2:L1000
        sheet.sort((1, 'asc'), range=f"A2:L{end_row + 100}")
